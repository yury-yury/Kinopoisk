...
def movie_dao():
    ...
    movie_dao.get_all_by_page = MagicMock(return_value=[movie_1, movie_2, movie_3])
    movie_dao.get_all_by_page_new = MagicMock(return_value=[movie_1, movie_2, movie_3])
    movie_dao.get_all = MagicMock(return_value=[movie_1, movie_2, movie_3])
    movie_dao.get_all_new = MagicMock(return_value=[movie_1, movie_2, movie_3])
...
class TestMovieService():
    ...
    def test_get_all_new(self):
        """
        The function contains unit tests to check the operability of the "get_all_new" method
        of the MovieService class.
        """
        movies = self.movie_service.get_all_new()

        assert len(movies) > 0
        assert movies[0] is not None
        assert type(movies) == list

    def test_get_all_by_page(self):
        """
        The function contains unit tests to check the operability of the "get_all" method
        of the MovieService class.
        """
        movies = self.movie_service.get_all_by_page(1)

        assert len(movies) > 0
        assert movies[0] is not None
        assert type(movies) == list

    def test_get_all_by_page_new(self):
        """
        The function contains unit tests to check the operability of the "get_all_by_page_new" method
        of the MovieService class.
        """
        movies = self.movie_service.get_all_by_page_new(1)
